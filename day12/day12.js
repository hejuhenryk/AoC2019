"use strict";
const scan = [{ x: -10, y: -13, z: 7 }, { x: 1, y: 2, z: 1 }, { x: -15, y: -3, z: 13 }, { x: 3, y: 7, z: -4 }]; // input
const gcd = (a, b) => !b ? a : gcd(b, a % b);
const lcm = (a, b) => (a * b) / gcd(a, b);
const calcEnergy = (vector) => {
    let energy = 0;
...
const updateVelocity = (moonOne, moonTwo, velocityOne, velocityTwo, axis) => {
...
const systemEnergy = (moons, steps) => {
    ...
    while (moves <= steps) {
        let totalEnergy = 0;
        velocitys.forEach((velocity, index) => {
            let kineticEnergy = 0;
...
*/
const findPeriodOfAxis = (moons, axis) => {
    let step = 0;

...
const startShooting = (shootingOrder: Shoot[], counter = 0) => {
...
const getDistance = (pointOne: string, pointTwo: string): number => {
    let distance = -1
...
const getAngle = (coordinateOne: Point, coordinateTwo: Point) => {
    let angle = 0;
...
const canStationSee = (station: string, asteroid: string, asteroidsDict: {}): boolean => {
    let canSee = true
    if (asteroid !== station) {
...
const doVectors = (a: Vector ,b: Vector, sign: Sign) => a.map( (element, index) => element + sign*b[index] ) 

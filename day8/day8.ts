...
const devTolayers = (data: string, pictureSize:{wide: number; tall: number}) => {
    const layers: string[] = []
...
const howManyNdigits = (str: string, N: number) => {
    let Ndigits = 0
    for( const i of str){
        if (+i===N) Ndigits++ ;
    }
...
.then( layers => {
    let layerFewestZeros: number | undefined = undefined
...
.then( massage => {
    ...
    massageArray.forEach( line => {
        console.log(line.split('').map((h: string) =>+h === 0?' ':+h === 1 ?'|':'*').join(''))
    })
})